...
def test_build_book_url_uses_cache_epub_pattern() -> None:
    book = make_book(book_id=1234)
    url = downloader._build_book_url(book)
    assert url == "https://www.gutenberg.org/cache/epub/1234/pg1234.txt"


def test_download_book_cache_hit(monkeypatch, tmp_path: Path) -> None:
    book = make_book()
...
def test_download_book_success(monkeypatch, tmp_path: Path) -> None:
    book = make_book()
...
def test_download_book_http_error(monkeypatch, tmp_path: Path) -> None:
    book = make_book()
    response = MagicMock()
...
def test_download_book_request_exception(monkeypatch, tmp_path: Path) -> None:
    book = make_book()
...
def test_download_all_invokes_each_book(monkeypatch, tmp_path: Path) -> None:
